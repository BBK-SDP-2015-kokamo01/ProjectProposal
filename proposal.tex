...
\begin{figure}[h!]
  ...
  \vspace{\baselineskip}
  \centering
    \includegraphics[width=0.5\textwidth]{system5.png}
\end{figure}
\begin{enumerate}

   \item \textbf{RFID Labeled Groceries}
   	\begin{flushleft}Groceries will be tagged using the correct class of RFID. Research will be carried out to understand the limitations and capabilities of available tags. Cost, range and compatibility will be analysed to select the most applicable tag for the use-case.
  	\end{flushleft}
	  \vspace{\baselineskip}

   \item \textbf{Gateway-RFID Interogator}
   	\begin{flushleft}A device which acknowledges the existence of the tagged items and routes the information in the tags to the server. sources will be obtained form existing systems. The reader will have a unique identifier, signifying the fridge. As Smartphones are equipped with RFID readers for the prototype a phone will act as a reader. Proof of concept 
  	\end{flushleft}
  
   \item \textbf{Cloud Based Web Server \& Database}
   	\begin{flushleft}Using AWS' cloud based API's and mobile development SDK. Programming language Java
  	\end{flushleft}
...
What do you refer to by the cloud/disk icon in the middle (is it a database, a web server, an API, etc).
\paragraph{Agile}
